...
let MXKROOMDATASOURCE_PAGINATION_LIMIT_AROUND_INITIAL_EVENT: UInt = 30
...
class RoomViewModel: ObservableObject {
    ...
    /// The default value is 30.
    var paginationLimitAroundInitialEvent: UInt = 0
    ...
    var filterMessagesWithURL: Bool {
        ...
        set(filterMessagesWithURL) {
            ...
            if isLive && room != nil {
                ...
                refreshEventListeners(_filterMessagesWithURL ? [kMXEventTypeStringRoomMessage] : AppSettings.shared.allEventTypesForMessages as [String]?)
            }
        }
    }
    ...
    func destroy() {
        print(">[RoomViewModel] Destroy room id:", roomId as Any)
        ...
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name.mxSessionVirtualRoomsDidChange, object: nil)
    ...
    init(roomId: String?, andMatrixSession matrixSession: MXSession?) {
        ...
        eventFormatter?.eventTypesFilterForMessages = AppSettings.shared.eventsFilterForMessages as [String]?
    ...
    func limitMemoryUsage(_ maxBubbleNb: Int) {
        ...
        if bubbleCount > maxBubbleNb {
            ...
            for index in 0..<outgoingMessages!.count {
                ...
                if outgoingMessage?.sentState == MXEventSentStateSending || outgoingMessage?.sentState == MXEventSentStatePreparing || outgoingMessage?.sentState == MXEventSentStateEncrypting || outgoingMessage?.sentState == MXEventSentStateUploading {
                    print(">[RoomViewModel] cancel limitMemoryUsage because some messages are being sent")
                    return
                }
            }
    ...
    func resetNotifying(_ notify: Bool) {
        ...
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name.mxRoomInitialSync, object: nil)
        
        var lockQueue = DispatchQueue(label: "eventsToProcess")
        lockQueue.sync {
            eventsToProcess?.removeAll()
        }
        
        // Suspend the reset operation if some events is under processing
        lockQueue = DispatchQueue(label: "eventsToProcessSnapshot")
        lockQueue.sync {
            eventsToProcessSnapshot = nil
            bubblesSnapshot = nil
            
            var lockQueue = DispatchQueue(label: "bubbles")
            lockQueue.sync {
            ...
            lockQueue = DispatchQueue(label: "eventIdToBubbleMap")
            lockQueue.sync {
        ...
        //TODO: - Refresh the UI. Seems it has to be @Published var
    }
    ...
    func didMXSessionStateChange() {
        guard let mxSession = mxSession else { return }
        
        if mxSession.state.storeDataReady == false {
                ...
                if self.room != nil {
                    ...
                    if isLive {
                        ...
                        self.room?.liveTimeline({ [self] liveTimeline in
                            ...
                            roomDidFlushDataNotificationObserver = NotificationCenter.default.addObserver(forName: NSNotification.Name.mxRoomDidFlushData, object: nil, queue: OperationQueue.main, using: { [self] notif in
                            ...
                            refreshEventListeners(filterMessagesWithURL ? [kMXEventTypeStringRoomMessage] : AppSettings.shared.allEventTypesForMessages as [String]?)
                            ...
                            if membership == .unknown || membership == .invite {
                                ...
                                NotificationCenter.default.addObserver(self, selector: #selector(didMXRoomInitialSynced(_:)), name: NSNotification.Name.mxRoomInitialSync, object: self.room)
                            }
                        })
                        ...
                        if secondaryRoom == nil && secondaryRoomId != nil {
                            ...
                            if let secondaryRoom = secondaryRoom {
                                secondaryRoom.liveTimeline({ [self] liveTimeline in
                                    ...
                                    refreshSecondaryEventListeners(secondaryRoomEventTypes as [String]?)
                                    ...
                                    if membership == .unknown || membership == .invite {
                                        ...
                                        // We have to observe here 'kMXRoomInitialSyncNotification' to reload room data when room sync is done.
                                        NotificationCenter.default.addObserver(self, selector: #selector(didMXRoomInitialSynced(_:)), name: NSNotification.Name.mxRoomInitialSync, object: secondaryRoom)
                                    }
                                })
                            }
                        }
                        ...
                        timeline?.resetPaginationAroundInitialEvent(withLimit: paginationLimitAroundInitialEvent) { [unowned self] result in
                            switch result {
                            case .success():
                            ...
                            case .failure(let error):
                                ...
                                print(">[RoomViewModel] Failed to resetPaginationAroundInitialEventWithLimit")
                    ...
                    print(">[RoomViewModel] Warning: The user does not know the room %@", roomId as Any)
            ...
            if self.room != nil && mxSession.state == .running {
                ...
                self.room?.state({ [self] roomState in
                    if roomState?.relatedGroups.count != 0 {
                        NotificationCenter.default.addObserver(self, selector: #selector(didMXSessionUpdatePublicisedGroups(forUsers:)), name: NSNotification.Name.mxSessionDidUpdatePublicisedGroupsForUsers, object: mxSession)
                        
                        // Get a fresh profile for all the related groups. Trigger a table refresh when all requests are done.
                        var count = roomState?.relatedGroups.count
                            ...
                            for groupId in relatedGroups {
                                ...
                                mxSession.updateGroupProfile(group, success: { [self] in
                                    ...
                                    if count == 0 {
                                        ...
                                        //TODO: - refresh @Published
                                    }
                                    
                                }, failure: { [self] error in
                                    
                                    print(">[RoomViewModel] group profile update failed", groupId)
    ...
    func refreshEventListeners(_ liveEventTypesFilterForMessages: [String]?) {
        ...
        if isLive {
            ...
            liveEventsListener = timeline?.listenToEvents(liveEventTypesFilterForMessages?.compactMap { MXEventType(identifier: $0) }) { [unowned self] event, direction, roomState in
                
                if direction == .forwards {
                    // Check for local echo suppression
                    var localEcho: MXEvent?
                    if room?.outgoingMessages().count != 0 && (event.sender == mxSession?.myUser.userId) {
                    ...
                    if secondaryRoom != nil {
                        ...
                        queueEvent(forProcessing: event, with: roomState, direction: .forwards)
                        processQueuedEvents({ _,_ in })
                    }
                }
            }
            
            receiptsListener = timeline?.listenToEvents([.receipt]) { [self] event, direction, roomState in
            ...
            relatedGroupsListener = timeline?.listenToEvents([.init(identifier: kMXEventTypeStringRoomRelatedGroups)]) { [self] event, direction, roomState in
        ...
        redactionListener = timeline?.listenToEvents([.roomRedaction]) { [self] redactionEvent, direction, roomState in
                ...
                RoomViewModel.processingQueue().async(execute: { [self] in
                    ...
                    if let bubbleData = bubbleData {
                        ...
                        lockQueue.sync {
                            ...
                            guard let events = bubbleData.events else { return }
                        ...
                        if hasChanged {
                            ...
                            DispatchQueue.main.async(execute: { [self] in
                                ...
                                // TODO: - Need to update data here (reloadData was here)
    ...
    func refreshSecondaryEventListeners(_ liveEventTypesFilterForMessages: [String]?) {
        // Remove the existing listeners
        ...
        if isLive {
            // Register a new one with the requested filter
            secondaryLiveEventsListener = secondaryTimeline?.listenToEvents(liveEventTypesFilterForMessages?.compactMap { MXEventType(identifier: $0) }) { [unowned self] event, direction, roomState in
                if direction == .forwards {
                    // Check for local echo suppression
                    var localEcho: MXEvent?
                    if secondaryRoom?.outgoingMessages().count != 0 && (event.sender == mxSession?.myUserId) {
        ...
        // Register a listener to handle redaction which can affect live and past timelines
        secondaryRedactionListener = secondaryTimeline?.listenToEvents([.roomRedaction]) { [unowned self] redactionEvent, direction, roomState in
    ...
    func listenTypingNotifications() {
        ...
        typingNotifListener = timeline?.listenToEvents([.typing]) { [unowned self] event, direction, roomState in
            ...
            if direction == .forwards {
                ...
                let index = typingUsers?.firstIndex(of: mxSession!.myUser.userId)
                if index != nil {
    ...
    func cellData(at index: Int) -> RoomBubbleCellProtocol? {
        weak var bubbleData: RoomBubbleCellProtocol?
        let lockQueue = DispatchQueue(label: "bubbles")
        lockQueue.sync {
    ...
    /// - Returns: the cell data
    func cellDataOfEvent(withEventId eventId: String?) -> RoomBubbleCellProtocol? {
        var bubbleData: RoomBubbleCellProtocol?
        let lockQueue = DispatchQueue(label: "eventIdToBubbleMap")
        lockQueue.sync {
            bubbleData = eventIdToBubbleMap?[eventId ?? ""] as? RoomBubbleCellProtocol
        }
        return bubbleData
    }
    ...
    func indexOfCellData(withEventId eventId: String?) -> Int? {
        guard let bubbleData = eventIdToBubbleMap?[eventId ?? ""] as? RoomBubbleCellProtocol else { return nil }
        return bubbles?.firstIndex(where: { $0.events == bubbleData.events })
    }
    ...
    func paginate(_ numItems: Int, direction: MXTimelineDirection, onlyFromStore: Bool, success: @escaping (_ addedCellNumber: Int) -> Void, failure: @escaping (_ error: Error?) -> Void) {
        ...
        if state != .ready || ((room?.summary.membership == .unknown || room?.summary.membership == .invite) && (roomState?.historyVisibility.identifier != kMXRoomHistoryVisibilityWorldReadable)) {
        ...
        if paginationRequest != nil || secondaryPaginationRequest != nil {
            print(">[RoomViewModel] paginate: a pagination is already in progress")
            failure(nil)
            return
        }
        
        if false == canPaginate(direction) {
            print(">[RoomViewModel] paginate: No more events to paginate")
            success(0)
        }
        ...
        paginationListener = timeline?.listenToEvents(filterMessagesWithURL ?
                                                      [.roomMessage] : AppSettings.shared.allEventTypesForMessages?.compactMap { MXEventType(identifier: $0 as String)}) { [unowned self] event, direction2, roomState in
        ...
        paginationRequest = timeline?.paginate(UInt(numItems), direction: direction, onlyFromStore: onlyFromStore) { [unowned self] result in
            ...
            case .success():
                ...
                processQueuedEvents({ addedHistoryCellNb, addedLiveCellNb in
                    ...
                    addedCellNb += (direction == .backwards) ? addedHistoryCellNb : addedLiveCellNb
                    dispatchGroup.leave()
                    
                })
            case .failure(let error):
                
                print(">[RoomViewModel] paginateBackMessages fails")
        ...
        if let secondaryTimeline = secondaryTimeline {
            // Define a new listener for this pagination
            secondaryPaginationListener = secondaryTimeline.listenToEvents(secondaryRoomEventTypes?.compactMap { MXEventType(identifier: $0 as String)} ) { [unowned self] event, direction2, roomState in
            ...
            dispatchGroup.enter()
            // Launch the pagination
            secondaryPaginationRequest = secondaryTimeline.paginate(UInt(numItems), direction: direction, onlyFromStore: onlyFromStore) { [unowned self] result in
...
//                    }
        ...
        dispatchGroup.notify(qos: .default, flags: .noQoS, queue: .main) {
...
//        let html = htmlMessage(fromSanitizedText: sanitizedText)
        ...
        if isEmote {
            room?.sendEmote(sanitizedText ?? "", formattedText: html, localEcho: &localEchoEvent) { result in
                switch result {
            ...
            room?.sendTextMessage(sanitizedText ?? "", formattedText: html, localEcho: &localEchoEvent) { result in
                switch result {
                case .success(let text):
                    success(text)
                case .failure(let error):
        ...
        if let localEchoEvent = localEchoEvent {
            ...
            queueEvent(forProcessing: localEchoEvent, with: roomState, direction: .forwards)
            processQueuedEvents({ _,_ in })
        }
    }
    ...
    func sendReplyToEvent(
        ...
        guard let eventToReply = event(withEventId: eventIdToReply) else { return }
...
//        let html = htmlMessage(fromSanitizedText: sanitizedText)
        ...
        let stringLocalizations = MXSendReplyEventDefaultStringLocalizations()
        
        room?.sendReply(to: eventToReply, textMessage: sanitizedText ?? "", formattedTextMessage: html, stringLocalizations: stringLocalizations, localEcho: &localEchoEvent) { result in
            switch result {
            case .success(let text):
                success(text)
            case .failure(let error):
...
//    }
    ...
    func send(_ image: UIImage?, success: @escaping (String?) -> Void, failure: @escaping (Error?) -> Void) {
        ...
        if room?.summary.isEncrypted ?? false {
            ...
            thumbnail = Tools.reduce(image, toFitIn: CGSize(width: 800, height: 600))
            if thumbnail == image {
                thumbnail = nil
            }
        }
    ...
    func sendVideoAsset(_ videoAsset: AVAsset?, withThumbnail videoThumbnail: UIImage?, success: @escaping (String?) -> Void, failure: @escaping (Error?) -> Void) {
        ...
        room?.sendVideo(asset: videoAsset, thumbnail: videoThumbnail, localEcho: &localEchoEvent) { result in
            switch result {
    ...
    /// the event id of the event generated on the homeserver
    ///   - failure: A block object called when the operation fails.
    func sendAudioFile(_ audioFileLocalURL: URL?, mimeType: String, success: @escaping (String?) -> Void, failure: @escaping (Error?) -> Void) {
        var localEchoEvent: MXEvent? = nil
        
        guard let audioFileLocalURL = audioFileLocalURL else { return }
        
        room?.sendAudioFile(localURL: audioFileLocalURL, mimeType: mimeType, localEcho: &localEchoEvent) { result in
            switch result {
    ...
    func sendVoiceMessage(
        ...
        room?.sendVoiceMessage(localURL: audioFileLocalURL, mimeType: mimeType, duration: UInt(duration), samples: samples, localEcho: &localEchoEvent) { result in
            switch result {
    ...
    func sendFile(_ fileLocalURL: URL?, mimeType: String?, success: @escaping (String?) -> Void, failure: @escaping (Error?) -> Void) {
        ...
        guard let fileLocalURL = fileLocalURL else {return}
        
        room?.sendFile(localURL: fileLocalURL, mimeType: mimeType!, localEcho: &localEchoEvent) { result in
            switch result {
    ...
    func sendMessage(withContent msgContent: [String : Any]?, success: @escaping (String?) -> Void, failure: @escaping (Error?) -> Void) {
        guard let msgContent = msgContent else {return}

        var localEchoEvent: MXEvent? = nil
        
        // Make the request to the homeserver
        room?.sendMessage(withContent: msgContent, localEcho: &localEchoEvent) { result in
            switch result {
    ...
    func sendEventOfType(_ eventTypeString: MXEventTypeString, content msgContent: [String : Any]?, success: @escaping (_ eventId: String?) -> Void, failure: @escaping (_ error: Error?) -> Void) {
        ...
        // Make the request to the homeserver
        room?.sendEvent(MXEventType(identifier: eventTypeString as String), content: msgContent, localEcho: &localEchoEvent) { result in
            switch result {
    ...
    func resendEvent(withEventId eventId: String?, success: @escaping (String?) -> Void, failure: @escaping (Error?) -> Void) {
        ...
        print(">[RoomViewModel] resendEventWithEventId. EventId:", event?.eventId as Any)
        ...
        if event?.wireType == kMXEventTypeStringRoomEncrypted {
            ...
            room?.sendEvent(MXEventType(identifier: kMXEventTypeStringRoomEncrypted), content: event!.wireContent, localEcho: &event) { result in
                switch result {
                case .success(let event):
                    success(event)
                case .failure(let error):
            ...
            if (msgType == kMXMessageTypeText) || (msgType == kMXMessageTypeEmote) {
                ...
                room?.sendMessage(withContent: event!.content, localEcho: &event) { result in
                    switch result {
                    case .success(let message):
                        success(message)
                    case .failure(let error):
                ...
                if contentURL != nil && contentURL?.hasPrefix(kMXMediaUploadIdPrefix) ?? false {
                    ...
                    if event?.content["info"] != nil {
                        mimetype = (event?.content["info"] as! [String : Any])["mimetype"] as? String
                    }
                    
                    guard let localImagePath = MXMediaManager.cachePath(forMatrixContentURI: contentURL, andType: mimetype, inFolder: roomId) else { return }
                    ...
                    if let image = image {
                        ...
                        print(">[RoomViewModel] resendEventWithEventId: Warning - Unable to resend room message of type:", msgType as Any)
                    }
                } else {
                    // Resend the Matrix event by reusing the existing echo
                    room?.sendMessage(withContent: event!.content, localEcho: &event) { result in
                ...
                var mimetype = (event?.content["info"] as! [String : Any])["mimetype"] as? String
                guard let localFilePath = MXMediaManager.cachePath(forMatrixContentURI: contentURL, andType: mimetype, inFolder: roomId) else {return}
                ...
                if !FileManager.default.fileExists(atPath: localFilePath) {
                    ...
                    print(">[RoomViewModel] resendEventWithEventId: Warning - Unable to resend voice message, invalid file path.")
                    return
                }
                ...
                if event?.isVoiceMessage() == true {
                    guard let extensibleAudio = event?.content[kMXMessageContentKeyExtensibleAudio] as? [String : Any] else { return }
                    let duration = extensibleAudio[kMXMessageContentKeyExtensibleAudioDuration] as? Int
                    let samples = extensibleAudio[kMXMessageContentKeyExtensibleAudioWaveform] as? [Float]
                    
                    sendVoiceMessage(localFileURL, mimeType: mimetype!, duration: duration ?? 0, samples: samples, success: success, failure: failure)
                } else {
                ...
                if contentURL != nil && contentURL?.hasPrefix(kMXMediaUploadIdPrefix) ?? false {
                    ...
                    print(">[RoomViewModel] resendEventWithEventId: Warning - Unable to resend attached video (upload was not complete)")
            ...
            failure(NSError(domain: MXKRoomDataSourceErrorDomain, code: RoomDataSourceError.resendInvalidMessageType.rawValue, userInfo: nil))
            print(">[RoomViewModel] RoomViewModel: Warning - Only resend of MXEventTypeRoomMessage is allowed. Event.type:", event?.type as Any)
        }
    }
    ...
    func event(withEventId eventId: String?) -> MXEvent? {
        ...
        let bubbleData = cellDataOfEvent(withEventId: eventId)
        if let bubbleData = bubbleData {
    ...
    func removeEvent(withEventId eventId: String?) {
        // First, retrieve the cell data hosting the event
        var bubbleData = cellDataOfEvent(withEventId: eventId)
        if let bubbleData = bubbleData {
        ...
        RoomViewModel.processingQueue().async(execute: { [self] in
            ...
            lockQueue.sync {
                for cellData in bubbles ?? [] {
                    guard let cellData = cellData as? RoomBubbleCellData else {
                        continue
                    }
            ...
            for eventId in readEventIds ?? [] {
                ...
                let cellData = cellDataOfEvent(withEventId: eventId) as? RoomBubbleCellData
    ...
    func update(_ cellData: RoomBubbleCellData?, withReadReceipts readReceipts: [MXReceiptData]?, forEventId eventId: String?) {
    ...
    func setState(_ newState: DataSourceState) {
        state = newState
    ...
    static func processingQueue() -> DispatchQueue {
