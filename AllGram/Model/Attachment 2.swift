...
class Attachment {
    ...
    /// - Returns: `MXKAttachment` instance.
    init?(event: MXEvent, andMediaManager mediaManager: MXMediaManager) {
        ...
        contentInfo = eventContent!["info"] as? [AnyHashable : Any]
        contentFile = MXEncryptedContentFile(fromJSON: eventContent!["file"] as? [AnyHashable : Any])
        ...
        if let thumbnailFile = thumbnailFile {
            ...
            let decryptAndCache: (() -> Void)? = { [self] in
                ...
                MXEncryptedAttachments.decryptAttachment(thumbnailFile, inputStream: instream, outputStream: outstream, success: { [self] in
                    ...
                    if err != nil {
                        print(">[getThumbnails] ERROR decrypting attachment!", (err! as NSError).userInfo)
                        onFailure(self, err)
                        return
                    }
                })
        ...
        prepare({ [self] in
            if isEncrypted {
                ...
                MXEncryptedAttachments.decryptAttachment(contentFile, inputStream: instream, outputStream: outstream, success: {
                    ...
                    if err != nil {
                        print(">[getData] Error decrypting attachment! %@", (err! as NSError).userInfo)
                        return
                    }
                })
            } else {
