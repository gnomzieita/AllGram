...
//        markdownToHTMLRenderer = MarkdownToHTMLRendererHardBreaks()
    ...
    func string(from event: MXEvent?, with roomState: MXRoomState?, error: inout EventFormatterError?) -> String? {
    ...
    func attributedString(from event: MXEvent?, with roomState: MXRoomState?, error: inout EventFormatterError?) -> NSAttributedString? {
        ...
        if isRedacted {
            ...
            if settings?.showRedactionsInRoomHistory == true {
                print(">[EventFormatter] Redacted event %@ (%@)", event?.description as Any, event?.redactedBecause as Any)
                ...
                let redactedReason = (event?.redactedBecause["content"] as? [String: Any])?["reason"] as? String
                if (redactedReason?.count ?? 0) != 0 {
                    if redactorId == mxSession?.myUserId {
                        let formatString = "notice_event_redacted_by_you - notice_event_redacted_reason"
                        redactedBy = String(format: formatString, redactedReason ?? "")
                    } else if redactedBy.count != 0 {
                        let formatString = "notice_event_redacted_by - notice_event_redacted_reason"
                        redactedBy = String(format: formatString, redactedBy, redactedReason ?? "")
                    } else {
                        redactedBy = String(format: "notice_event_redacted_reason", redactedReason ?? "")
                    }
                } else if redactorId == mxSession?.myUserId {
                    redactedBy = "notice_event_redacted_by_you"
                } else if redactedBy.count != 0 {
                    redactedBy = String(format: "notice_event_redacted_by", redactedBy)
                }

                redactedInfo = String(format: "notice_event_redacted", redactedBy)
            }
        }
        ...
        case .roomName:
            ...
            if (roomName?.count ?? 0) != 0 {
                if isEventSenderMyUser {
                    if isRoomDirect {
                        displayText = String(format: "notice_room_name_changed_by_you_for_dm", roomName ?? "")
                    } else {
                        displayText = String(format: "notice_room_name_changed_by_you", roomName ?? "")
                    }
                } else {
                    if isRoomDirect {
                        displayText = String(format: "notice_room_name_changed_for_dm", senderDisplayName ?? "", roomName ?? "")
                    } else {
                        displayText = String(format: "notice_room_name_changed", senderDisplayName ?? "", roomName ?? "")
                    }
                }
            } else {
                if isEventSenderMyUser {
                    if isRoomDirect {
                        displayText = "notice_room_name_removed_by_you_for_dm"
                    } else {
                        displayText = "notice_room_name_removed_by_you"
                    }
                } else {
                    if isRoomDirect {
                        displayText = String(format: "notice_room_name_removed_for_dm", senderDisplayName ?? "")
                    } else {
                        displayText = String(format: "notice_room_name_removed", senderDisplayName ?? "")
                    }
                }
            }
        case .roomTopic:
            ...
            if (roomTopic?.count ?? 0) != 0 {
                if isEventSenderMyUser {
                    displayText = String(format: "notice_topic_changed_by_you", roomTopic ?? "")
                } else {
                    displayText = String(format: "notice_topic_changed", senderDisplayName ?? "", roomTopic ?? "")
                }
            } else {
                if isEventSenderMyUser {
                    displayText = "notice_room_topic_removed_by_you"
                } else {
                    displayText = String(format: "notice_room_topic_removed", senderDisplayName ?? "")
                }
            }
        case .roomMember:
            ...
            if event?.isUserProfileChange() == true {
                ...
                if isRedacted {
                    ...
                    if isEventSenderMyUser {
                        displayText = String(format: "notice_profile_change_redacted_by_you", redactedInfo ?? "")
                    } else {
                        displayText = String(format: "notice_profile_change_redacted", senderDisplayName ?? "", redactedInfo ?? "")
                    }
                } else {
                    ...
                    if (displayname != nil || prevDisplayname != nil) && ((displayname == prevDisplayname) == false) {
                        if prevDisplayname == nil {
                            if isEventSenderMyUser {
                                displayText = String(format: "notice_display_name_set_by_you", displayname ?? "")
                            } else {
                                if let sender = event?.sender {
                                    displayText = String(format: "notice_display_name_set", sender, displayname ?? "")
                                }
                            }
                        } else if displayname == nil {
                            if isEventSenderMyUser {
                                displayText = "notice_display_name_removed_by_you"
                            } else {
                                if let sender = event?.sender {
                                    displayText = String(format: "notice_display_name_removed", sender)
                                }
                            }
                        } else {
                            if isEventSenderMyUser {
                                displayText = String(format: "notice_display_name_changed_from_by_you", prevDisplayname ?? "", displayname ?? "")
                            } else {
                                if let sender = event?.sender {
                                    displayText = String(format: "notice_display_name_changed_from", sender, prevDisplayname ?? "", displayname ?? "")
                                }
                            }
                        }
                    }
                    ...
                    if (prevAvatar != nil || avatar != nil) && ((avatar == prevAvatar) == false) {
                        if displayText != nil {
                            displayText = "\(displayText ?? "") notice_avatar_changed_too"
                        } else {
                            if isEventSenderMyUser {
                                displayText = "notice_avatar_url_changed_by_you"
                            } else {
                                displayText = String(format: "notice_avatar_url_changed", senderDisplayName ?? "")
                            }
                        }
                    }
                }
                ...
                if membership == "invite" {
                    if event?.content["third_party_invite"] != nil {
                        if event?.stateKey == mxSession?.myUserId {
                            if let content = (event?.content["third_party_invite"] as? [String: Any])?["display_name"] as? String {
                                displayText = String(format: "notice_room_third_party_registered_invite_by_you", content)
                            }
                        } else {
                            if let content = (event?.content["third_party_invite"] as? [String: Any])?["display_name"] as? String {
                                displayText = String(format: "notice_room_third_party_registered_invite", targetDisplayName ?? "", content)
                            }
                        }
                    } else {
                        if MXCallManager.isConferenceUser(event!.stateKey) {
                            if isEventSenderMyUser {
                                displayText = "notice_conference_call_request_by_you"
                            } else {
                                displayText = String(format: "notice_conference_call_request", senderDisplayName ?? "")
                            }
                        } else {
                            // The targeted member display name (if any) is available in content
                            if isEventSenderMyUser {
                                displayText = String(format: "notice_room_invite_by_you", targetDisplayName ?? "")
                            } else if targetDisplayName == mxSession?.myUserId {
                                displayText = String(format: "notice_room_invite_you", senderDisplayName ?? "")
                            } else {
                                if contentDisplayname != nil {
                                    targetDisplayName = contentDisplayname
                                }

                                displayText = String(format: "notice_room_invite", senderDisplayName ?? "", targetDisplayName ?? "")
                            }
                        }
                    }
                } else if membership == "join" {
                    if MXCallManager.isConferenceUser(event!.stateKey) {
                        displayText = "notice_conference_call_started"
                    } else {
                        // The targeted member display name (if any) is available in content
                        if isEventSenderMyUser {
                            displayText = "notice_room_join_by_you"
                        } else {
                            if contentDisplayname != nil {
                                targetDisplayName = contentDisplayname
                            }

                            displayText = String(format: "notice_room_join", targetDisplayName ?? "")
                        }
                    }
                } else if membership == "leave" {
                    var prevMembership: String? = nil
                    if event?.prevContent != nil {
                        prevMembership = event?.prevContent["membership"] as? String
                    }
                    ...
                    if event?.sender == event?.stateKey {
                        if MXCallManager.isConferenceUser(event!.stateKey) {
                            displayText = "notice_conference_call_finished"
                        } else {
                            if prevMembership != nil && (prevMembership == "invite") {
                                if isEventSenderMyUser {
                                    displayText = "notice_room_reject_by_you"
                                } else {
                                    displayText = String(format: "notice_room_reject", targetDisplayName ?? "")
                                }
                            } else {
                                if isEventSenderMyUser {
                                    displayText = "notice_room_leave_by_you"
                                } else {
                                    displayText = String(format: "notice_room_leave", targetDisplayName ?? "")
                                }
                            }
                        }
                        ...
                        if prevMembership == "invite" {
                            if isEventSenderMyUser {
                                displayText = String(format: "notice_room_withdraw_by_you", targetDisplayName ?? "")
                            } else {
                                displayText = String(format: "notice_room_withdraw", senderDisplayName ?? "", targetDisplayName ?? "")
                            }
                            if event?.content["reason"] != nil {
                                if let content = event?.content["reason"] as? String {
                                    displayText = (displayText ?? "") + String(format: "notice_room_reason", content)
                                }
                            }
                        } else if prevMembership == "join" {
                            if isEventSenderMyUser {
                                displayText = String(format: "notice_room_kick_by_you", targetDisplayName ?? "")
                            } else {
                                displayText = String(format: "notice_room_kick", senderDisplayName ?? "", targetDisplayName ?? "")
                            }
                            ...
                            if isEventSenderMyUser {
                                displayText = String(format: "notice_room_unban_by_you", targetDisplayName ?? "")
                            } else {
                                displayText = String(format: "notice_room_unban", senderDisplayName ?? "", targetDisplayName ?? "")
                            }
                        }
                    }
                    ...
                    if isEventSenderMyUser {
                        displayText = String(format: "notice_room_ban_by_you", targetDisplayName ?? "")
                    } else {
                        displayText = String(format: "notice_room_ban", senderDisplayName ?? "", targetDisplayName ?? "")
                    }
                    if event?.content["reason"] != nil {
                        if let content = event?.content["reason"] as? String {
                            displayText = (displayText ?? "") + String(format: "notice_room_reason", content)
        ...
        case .roomCreate:
            ...
            if let creatorId = creatorId {
                if creatorId == mxSession?.myUserId {
                    if isRoomDirect {
                        displayText = "notice_room_created_by_you_for_dm"
                    } else {
                        displayText = "notice_room_created_by_you"
                    }
                } else {
                    if isRoomDirect {
                        displayText = String(format: "notice_room_created_for_dm", (roomState != nil ? roomState?.members.memberName(creatorId) : creatorId) ?? "")
                    } else {
                        displayText = String(format: "notice_room_created", (roomState != nil ? roomState?.members.memberName(creatorId) : creatorId) ?? "")
                    }
                }
                // Append redacted info if any
                if let redactedInfo = redactedInfo {
            ...
            if let joinRule = joinRule {
                if event?.sender == mxSession?.myUserId {
                    if joinRule == kMXRoomJoinRulePublic {
                        if isRoomDirect {
                            displayText = "notice_room_join_rule_public_by_you_for_dm"
                        } else {
                            displayText = "notice_room_join_rule_public_by_you"
                        }
                    } else if joinRule == kMXRoomJoinRuleInvite {
                        if isRoomDirect {
                            displayText = "notice_room_join_rule_invite_by_you_for_dm"
                        } else {
                            displayText = "notice_room_join_rule_invite_by_you"
                        }
                    }
                } else {
                    ...
                    if joinRule == kMXRoomJoinRulePublic {
                        if isRoomDirect {
                            displayText = String(format: "notice_room_join_rule_public_for_dm", displayName ?? "")
                        } else {
                            displayText = String(format: "notice_room_join_rule_public", displayName ?? "")
                        }
                    } else if joinRule == kMXRoomJoinRuleInvite {
                        if isRoomDirect {
                            displayText = String(format: "notice_room_join_rule_invite_for_dm", displayName ?? "")
                        } else {
                            displayText = String(format: "notice_room_join_rule_invite", displayName ?? "")
                        }
                    }
                }
                ...
                if displayText == nil {
                    ...
                    displayText = String(format: "notice_room_join_rule", joinRule)
                }
        ...
        case .roomPowerLevels:
            if isRoomDirect {
                displayText = "notice_room_power_level_intro_for_dm"
            } else {
                displayText = "notice_room_power_level_intro"
            }
            
            if let users = event?.content["users"] as? [String: Any] {
                for key in users.keys {
                    if let object = users[key] {
                        displayText = String(format: "%@\n\u{2022} %@: %@", displayText!, key, users[key] as! CVarArg)
                    }
                }
            }
            ...
            if event?.content["users_default"] != nil {
                if let content = event?.content["users_default"] {
                    displayText = String(format: "%@\n\u{2022} %@: %@", displayText!, "default", event?.content["users_default"] as! CVarArg)
                }
            }
            ...
            if event?.content["ban"] != nil {
                if let content = event?.content["ban"] {
                    displayText = String(format: "%@\n\u{2022} ban: %@", displayText!, event?.content["ban"] as! CVarArg)
                }
            }
            if event?.content["kick"] != nil {
                if let content = event?.content["kick"] {
                    displayText = String(format: "%@\n\u{2022} kick: %@", displayText!, event?.content["kick"] as! CVarArg)
                }
            }
            if event?.content["redact"] != nil {
                if let content = event?.content["redact"] {
                    displayText = String(format: "%@\n\u{2022} redact: %@", displayText!, event?.content["redact"] as! CVarArg)
                }
            }
            if event?.content["invite"] != nil {
                if let content = event?.content["invite"] {
                    displayText = String(format: "%@\n\u{2022} invite: %@", displayText!, event?.content["invite"] as! CVarArg)
                }
            }

            displayText = "\(displayText ?? "")\n notice_room_power_level_event_requirement"

            if let events = event?.content["events"] as? [String: Any] {
                for key in events.keys {
                    if let object = events[key] {
                        displayText = String(format: "%@\n\u{2022} %@: %@", displayText!, key, events[key] as! CVarArg)
                    }
                }
            }
            if event?.content["events_default"] != nil {
                if let content = event?.content["events_default"] {
                    displayText = String(format: "%@\n\u{2022} %@: %@", displayText!, "events_default", event?.content["events_default"] as! CVarArg)
                }
            }
            if event?.content["state_default"] != nil {
                if let content = event?.content["state_default"] {
                    displayText = String(format: "%@\n\u{2022} %@: %@", displayText!, "state_default", event?.content["state_default"] as! CVarArg)
                }
            }
        ...
        case .roomAliases:
            let aliases = event?.content["aliases"] as? [String]
            if let aliases = aliases {
                if isRoomDirect {
                    displayText = String(format: "notice_room_aliases_for_dm", aliases)
                } else {
                    displayText = String(format: "notice_room_aliases", aliases)
                }
                // Append redacted info if any
                if let redactedInfo = redactedInfo {
                    displayText = "\(displayText ?? "")\n \(redactedInfo)"
        ...
        case .roomRelatedGroups:
            let groups = event?.content["groups"] as? [String]
            if let groups = groups {
                displayText = String(format: "notice_room_related_groups", groups)
                // Append redacted info if any
                if let redactedInfo = redactedInfo {
                    displayText = "\(displayText ?? "")\n \(redactedInfo)"
                }
        ...
        case .roomEncrypted:
            ...
            if isRedacted {
                ...
                if event?.decryptionError != nil {
                    ...
                    if ((event?.decryptionError as! NSError).domain == MXDecryptingErrorDomain) && AppSettings.shared.hideUndecryptableEvents {
                        //  Hide this event, it cannot be decrypted
                        displayText = nil
                    } else if ((event?.decryptionError as! NSError).domain == MXDecryptingErrorDomain) && (event?.decryptionError as! NSError).code == MXDecryptingErrorUnknownInboundSessionIdCode.rawValue {
                        // Make the unknown inbound session id error description more user friendly
                        errorDescription = "notice_crypto_error_unknown_inbound_session_id"
                    } else if ((event?.decryptionError as! NSError).domain == MXDecryptingErrorDomain) && (event?.decryptionError as! NSError).code == MXDecryptingErrorDuplicateMessageIndexCode.rawValue {
                        // Hide duplicate message warnings
                        print(">[EventFormatter] Warning: Duplicate message with error description %@", event?.decryptionError as Any)
                        displayText = nil
                    } else {
                    ...
                    if let errorDescription = errorDescription {
                        displayText = String(format: "notice_crypto_unable_to_decrypt", errorDescription)
                    }
                } else {
                    displayText = "notice_encrypted_message"
                }
            }
        case .roomEncryption:
            ...
            if algorithm == kMXCryptoMegolmAlgorithm {
                if isEventSenderMyUser {
                    displayText = "notice_encryption_enabled_ok_by_you"
                } else {
                    displayText = String(format: "notice_encryption_enabled_ok", senderDisplayName ?? "")
                }
            } else {
                if isEventSenderMyUser {
                    displayText = String(format: "notice_encryption_enabled_unknown_algorithm_by_you", algorithm ?? "")
                } else {
                    displayText = String(format: "notice_encryption_enabled_unknown_algorithm", senderDisplayName ?? "", algorithm ?? "")
                }
            }
        case .roomHistoryVisibility:
            if isRedacted {
                ...
                let historyVisibility = event?.content["history_visibility"] as? MXRoomHistoryVisibility

                if historyVisibility != nil {
                    if historyVisibility == .worldReadable {
                        if !isRoomDirect {
                            if isEventSenderMyUser {
                                displayText = "notice_room_history_visible_to_anyone_by_you"
                            } else {
                                displayText = String(format: "notice_room_history_visible_to_anyone", senderDisplayName ?? "")
                            }
                        }
                    } else if historyVisibility == .shared {
                        if isEventSenderMyUser {
                            if isRoomDirect {
                                displayText = "notice_room_history_visible_to_members_by_you_for_dm"
                            } else {
                                displayText = "notice_room_history_visible_to_members_by_you"
                            }
                        } else {
                            if isRoomDirect {
                                displayText = String(format: "notice_room_history_visible_to_members_for_dm", senderDisplayName ?? "")
                            } else {
                                displayText = String(format: "notice_room_history_visible_to_members", senderDisplayName ?? "")
                            }
                        }
                    } else if historyVisibility == .invited {
                        if isEventSenderMyUser {
                            if isRoomDirect {
                                displayText = "notice_room_history_visible_to_members_from_invited_point_by_you_for_dm"
                            } else {
                                displayText = "notice_room_history_visible_to_members_from_invited_point_by_you"
                            }
                        } else {
                            if isRoomDirect {
                                displayText = String(format: "notice_room_history_visible_to_members_from_invited_point_for_dm", senderDisplayName ?? "")
                            } else {
                                displayText = String(format: "notice_room_history_visible_to_members_from_invited_point", senderDisplayName ?? "")
                            }
                        }
                    } else if historyVisibility == .joined {
                        if isEventSenderMyUser {
                            if isRoomDirect {
                                displayText = "notice_room_history_visible_to_members_from_joined_point_by_you_for_dm"
                            } else {
                                displayText = "notice_room_history_visible_to_members_from_joined_point_by_you"
                            }
                        } else {
                            if isRoomDirect {
                                displayText = String(format: "notice_room_history_visible_to_members_from_joined_point_for_dm", senderDisplayName ?? "")
                            } else {
                                displayText = String(format: "notice_room_history_visible_to_members_from_joined_point", senderDisplayName ?? "")
                            }
                        }
                    }
                }
        ...
        case .roomMessage:
            ...
            if isRedacted {
                ...
                displayText = redactedInfo
            } else if event?.isEditEvent() == true {
                return nil
            } else {
                let msgtype = event?.content["msgtype"] as? String
                ...
                if body != nil {
                    if msgtype == kMXMessageTypeImage {
                        body = body ?? "notice_image_attachment"
                        // Check attachment validity
                        if !isSupportedAttachment(event) {
                            print(">[EventFormatter] Warning: Unsupported attachment %@", event?.description as Any)
                            body = "notice_invalid_attachment"
                            error = .unsupported
                        }
                    } else if msgtype == kMXMessageTypeAudio {
                        body = body ?? "notice_audio_attachment"
                        if !isSupportedAttachment(event) {
                            print(">[EventFormatter] Warning: Unsupported attachment %@", event?.description as Any)
                            if isForSubtitle || !settings!.showUnsupportedEventsInRoomHistory {
                                ...
                                if let description = event?.description {
                                    body = String(format: "notice_unsupported_attachment", description)
                                }
                            }
                            error = .unsupported
                        }
                    } else if msgtype == kMXMessageTypeVideo {
                        body = body ?? "notice_video_attachment"
                        if !isSupportedAttachment(event) {
                            print(">[EventFormatter] Warning: Unsupported attachment %@", event?.description as Any)
                            if isForSubtitle || !settings!.showUnsupportedEventsInRoomHistory {
                                body = "notice_invalid_attachment"
                        ...
                        body = body ?? "notice_location_attachment"
                        if !isSupportedAttachment(event) {
                            print(">[EventFormatter] Warning: Unsupported attachment %@", event?.description as Any)
                            if isForSubtitle || !settings!.showUnsupportedEventsInRoomHistory {
                                body = "notice_invalid_attachment"
                        ...
                        body = body ?? "notice_file_attachment"
                        // Check attachment validity
                        if !isSupportedAttachment(event) {
                            print(">[EventFormatter] Warning: Unsupported attachment %@", event?.description as Any)
                            body = "notice_invalid_attachment"
                    ...
                    if msgtype == kMXMessageTypeEmote {
                        ...
                        if relatesTo?["m.in_reply_to"] is [AnyHashable : Any] {
                            attributedDisplayText?.enumerateAttributes(in: NSRange(location: 0, length: attributedDisplayText?.length ?? 0), options: .reverse, using: { value, range, stop in
        ...
        case .roomMessageFeedback:
            ...
            if type != nil && eventId != nil {
                displayText = String(format: "notice_feedback", eventId ?? "", type ?? "")
                // Append redacted info if any
                if let redactedInfo = redactedInfo {
                    displayText = "\(displayText ?? "") \(redactedInfo)"
                }
        ...
        case .roomRedaction:
            ...
            if isEventSenderMyUser {
                displayText = String(format: "notice_redaction_by_you", eventId ?? "")
            } else {
                displayText = String(format: "notice_redaction", senderDisplayName ?? "", eventId ?? "")
            }
        case .roomThirdPartyInvite:
            ...
            if let displayname = displayname {
                if isEventSenderMyUser {
                    if isRoomDirect {
                        displayText = String(format: "notice_room_third_party_invite_by_you_for_dm", displayname)
                    } else {
                        displayText = String(format: "notice_room_third_party_invite_by_you", displayname)
                    }
                } else {
                    if isRoomDirect {
                        displayText = String(format: "notice_room_third_party_invite_for_dm", senderDisplayName ?? "", displayname)
                    } else {
                        displayText = String(format: "notice_room_third_party_invite", senderDisplayName ?? "", displayname)
                    }
                }
            } else {
                ...
                if isEventSenderMyUser {
                    if isRoomDirect {
                        displayText = String(format: "notice_room_third_party_revoked_invite_by_you_for_dm", displayname ?? "")
                    } else {
                        displayText = String(format: "notice_room_third_party_revoked_invite_by_you", displayname ?? "")
                    }
                } else {
                    if isRoomDirect {
                        displayText = String(format: "notice_room_third_party_revoked_invite_for_dm", senderDisplayName ?? "", displayname ?? "")
                    } else {
                        displayText = String(format: "notice_room_third_party_revoked_invite", senderDisplayName ?? "", displayname ?? "")
                    }
                }
            }
        case .callInvite:
            ...
            if callInviteEventContent?.isVideoCall() == true {
                if isEventSenderMyUser {
                    displayText = "notice_placed_video_call_by_you"
                } else {
                    displayText = String(format: "notice_placed_video_call", senderDisplayName ?? "")
                }
            } else {
                if isEventSenderMyUser {
                    displayText = "notice_placed_voice_call_by_you"
                } else {
                    displayText = String(format: "notice_placed_voice_call", senderDisplayName ?? "")
                }
            }
        case .callAnswer:
            if isEventSenderMyUser {
                displayText = "notice_answered_video_call_by_you"
            } else {
                displayText = String(format: "notice_answered_video_call", senderDisplayName ?? "")
            }
        case .callHangup:
            if isEventSenderMyUser {
                displayText = "notice_ended_video_call_by_you"
            } else {
                displayText = String(format: "notice_ended_video_call", senderDisplayName ?? "")
            }
        case .callReject:
            if isEventSenderMyUser {
                displayText = "notice_declined_video_call_by_you"
            } else {
                displayText = String(format: "notice_declined_video_call", senderDisplayName ?? "")
            }
        case .sticker:
            // Is redacted?
            if isRedacted {
                ...
                if !isSupportedAttachment(event) {
                    print(">[EventFormatter] Warning: Unsupported sticker %@", event?.description as Any)
                    body = "notice_invalid_attachment"
                    error = .unsupported
                }

                displayText = body ?? "notice_sticker"
            }
        default:
        ...
        if attributedDisplayText == nil {
            print(">[EventFormatter] Warning: Unsupported event %@)", event?.description as Any)
            if settings?.showUnsupportedEventsInRoomHistory == true {
                if error == EventFormatterError.none {
                    error = .unsupported
                }
                ...
                switch error {
                case .unsupported:
                    shortDescription = "notice_error_unsupported_event"
                case .unexpected:
                    shortDescription = "notice_error_unexpected_event"
                case .unknownEventType:
                    shortDescription = "notice_error_unknown_event_type"
                default:
                    break
                }
    ...
    func attributedString(fromEvents events: [MXEvent]?, with roomState: MXRoomState?, error: EventFormatterError?) -> NSAttributedString? {
    ...
    func renderString(_ string: String?, for event: MXEvent?) -> NSAttributedString? {
        ...
        str.addAttribute(.foregroundColor, value: textColor(for: event) as Any, range: wholeString)
        str.addAttribute(.font, value: font(for: event) as Any, range: wholeString)
        ...
        if !((settings?.httpLinkScheme == "http") && (settings?.httpsLinkScheme == "https")) {
            ...
            for match in matches ?? [] {
                ...
                var url: URLComponents? = nil
                if let matchUrl = matchUrl {
                    url = URLComponents(url: matchUrl, resolvingAgainstBaseURL: false)
                }

                if let tmpUrl = url {
                    if tmpUrl.scheme == "http" {
                        url?.scheme = settings?.httpLinkScheme
                    } else if tmpUrl.scheme == "https" {
    ...
    func renderReply(to htmlString: String?, with roomState: MXRoomState?) -> String? {
        ...
        if inReplyToTextRange.location != NSNotFound {
            html = (html as NSString?)?.replacingCharacters(in: inReplyToTextRange, with: "notice_in_reply_to")
        }
    ...
    func renderString(_ string: String?, withPrefix prefix: String?, for event: MXEvent?) -> NSAttributedString? {
        ...
        if let prefix = prefix {
            ...
            str?.addAttribute(.foregroundColor, value: prefixTextColor as Any, range: prefixRange)
            str?.addAttribute(.font, value: prefixTextFont as Any, range: prefixRange)
