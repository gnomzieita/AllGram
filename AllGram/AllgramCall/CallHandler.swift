...
//        let str = call(for: viewKind)?.callerName
        ...
        callManager.placeCall(
            ...
            success: { [unowned self] mxCall in
                ...
                if let accessToken = room.session.credentials.accessToken {
                    NewApiManager.shared.recordCalling(roomId: room.roomId, isVideo: hasVideo, accessToken: accessToken)
    ...
    private func handlePushCall(fallbackSession: MXSession?, roomId: String, userName: String, event: MXEvent?, completion: @escaping () -> Void) {
    ...
    /// New
//    func makePushNotificationIncomingCall(fallbackSession: MXSession?, roomId: String, eventId: String, userName: String, completion: @escaping () -> Void) {
...
//
//                self?.handlePushCall(fallbackSession: fallbackSession, roomId: roomId, userName: userName, event: event, completion: completion)
//            },
//            failure: { [weak self] error in
//
//                self?.handlePushCall(fallbackSession: fallbackSession, roomId: roomId, userName: userName, event: nil, completion: completion)
//            }
//        )
//    }
...
//			return
//		}
//		guard let eventId = dict["eventId"] as? String else {
//			completion()
//			return
//		}
//		// get event?
...
//		} else {
            ...
            let CallKitConfiguration = MXCallKitConfiguration.init(name: (dict["displayName"] as? String ?? ""), ringtoneName: "ringtone.caf", iconName: nil, supportsVideo: dict["video"] as? Bool ?? false)
            
            var adapter:MXCallKitAdapter = MXCallKitAdapter.init(configuration: CallKitConfiguration)
        let call:MXCall = MXCall(roomId: dict["roomId"] as? String ?? "", andCallManager: (authSession?.callManager)!)
            ...
            case .connected:
                ...
                if call.isIncoming {
                    ...
                    if let accessToken = AuthViewModel.shared.session?.credentials.accessToken {
                        NewApiManager.shared.recordAnswering(roomId: call.room.roomId, accessToken: accessToken)
