...
//        let str = call(for: viewKind)?.callerName
	...
	private func makeDirectCall(room: AllgramRoom, hasVideo: Bool) {
        ...
        if let accessToken = room.session.credentials.accessToken {
            ApiManager.shared.recordCallHistory(roomId: room.roomId, isVideo: hasVideo, accessToken: accessToken)
...
private extension CallHandler {
    func startObserving() {
        ...
        let observer1 = nc.addObserver(forName: NSNotification.Name(kMXCallManagerNewCall),
