...
/// The notification object is the `MXKRoomDataSource` instance./// Posted when the data source has failed to paginate around an event.
...
class MXKRoomDataSource: MXKDataSource, UITableViewDataSource {
    ...
    private var bubblesSnapshot: [MXKRoomBubbleCellDataStoring?]?
    ...
    private weak var collapsableSeriesAtStart: MXKRoomBubbleCellDataStoring?
    /// If any, the non terminated series of collapsable events at the end of self.bubbles.
    /// (Such series is determined by the cell data of its oldest event).
    private weak var collapsableSeriesAtEnd: MXKRoomBubbleCellDataStoring?
    ...
    var roomState: MXRoomState? {
        ...
        assert(timeline?.state, "[MXKRoomDataSource] Room state must be preloaded before accessing to MXKRoomDataSource.roomState")
    ...
    var attachmentsWithThumbnail: [AnyHashable]? {
        ...
        lockQueue.sync {
            for bubbleData in bubbles ?? [] {
                if bubbleData.isAttachmentWithThumbnail && bubbleData.attachment.type != MXKAttachmentTypeSticker && !bubbleData.showAntivirusScanStatus {
                    attachments.append(bubbleData.attachment)
                }
            }
        }
    ...
    var partialTextMessage: String? {
        get {
            return room?._partialTextMessage
        }
    ...
    private var _eventFormatter: MXKEventFormatter?
    var eventFormatter: MXKEventFormatter? {
        get {
            _eventFormatter
        }
        set(eventFormatter) {
    ...
    var filterMessagesWithURL: Bool {
        ...
        set(filterMessagesWithURL) {
            ...
            if isLive && room != nil {
                ...
                refreshEventListeners(_filterMessagesWithURL ? [kMXEventTypeStringRoomMessage] : MXKAppSettings.standard().allEventTypesForMessages)
            }
        }
    }
    ...
    class func load(withRoomId roomId: String?, andMatrixSession mxSession: MXSession?, onComplete: @escaping (_ roomDataSource: Any?) -> Void) {
        let roomDataSource = self.init(roomId: roomId, andMatrixSession: mxSession)
    ...
    ///   - mxSession: the Matrix session to get data from.
    ///   - onComplete: a block providing the newly created instance.
    class func load(withRoomId roomId: String?, initialEventId: String?, andMatrixSession mxSession: MXSession?, onComplete: @escaping (_ roomDataSource: Any?) -> Void) {
        let roomDataSource = self.init(roomId: roomId, initialEventId: initialEventId, andMatrixSession: mxSession)
    ...
    class func load(with peekingRoom: MXPeekingRoom?, andInitialEventId initialEventId: String?, onComplete: @escaping (_ roomDataSource: Any?) -> Void) {
        let roomDataSource = self.init(peekingRoom: peekingRoom, andInitialEventId: initialEventId)
    ...
    class func ensureSessionState(for roomDataSource: MXKRoomDataSource?, initialEventId: String?, andMatrixSession mxSession: MXSession?, onComplete: @escaping (_ roomDataSource: Any?) -> Void) {
        ...
        if let state = mxSession?.state {
            if state >= MXSessionStateStoreDataReady {
                ...
                let sessionStateObserver = NotificationCenter.default.addObserver(forName: kMXSessionStateDidChangeNotification, object: mxSession, queue: nil, using: { [self] note in
                    if state >= MXSessionStateStoreDataReady {
    ...
    class func ensureInitialEventExistence(for roomDataSource: MXKRoomDataSource?, initialEventId: String?, andMatrixSession mxSession: MXSession?, onComplete: @escaping (_ roomDataSource: Any?) -> Void) {
        ...
        if initialEventId == nil {
            ...
            self.finalizeRoomDataSource(roomDataSource as? MXKRoomDataSource, onComplete: onComplete)
            return
        }

        //  ensure event with id 'initialEventId' exists in the session store
        if mxSession?.store.eventExists(withEventId: initialEventId, inRoom: roomDataSource?.roomId) {
            self.finalizeRoomDataSource(roomDataSource as? MXKRoomDataSource, onComplete: onComplete)
        } else {
            ...
            let syncObserver = NotificationCenter.default.addObserver(forName: kMXSessionDidSyncNotification, object: mxSession, queue: nil, using: { [self] note in
    ...
    class func finalizeRoomDataSource(_ roomDataSource: MXKRoomDataSource?, onComplete: @escaping (_ roomDataSource: Any?) -> Void) {
    ...
    init(roomId: String?, andMatrixSession matrixSession: MXSession?) {
        super.init(matrixSession: matrixSession)
        ...
        let virtualRoomId = matrixSession?.virtualRoomOf(self.roomId)
        ...
        // Cell data
        registerCellDataClass(MXKRoomBubbleCellData.self, forCellIdentifier: kMXKRoomBubbleCellDataIdentifier)

        // Set default MXEvent -> NSString formatter
        eventFormatter = MXKEventFormatter(matrixSession: mxSession)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(eventDidChangeSentState(_:)), name: kMXEventDidChangeSentStateNotification, object: nil)
        // Listen to events decrypted
        NotificationCenter.default.addObserver(self, selector: #selector(eventDidDecrypt(_:)), name: kMXEventDidDecryptNotification, object: nil)
        // Listen to virtual rooms change
        NotificationCenter.default.addObserver(self, selector: #selector(virtualRoomsDidChange(_:)), name: kMXSessionVirtualRoomsDidChangeNotification, object: matrixSession)
    }
    ...
    func destroy() {
        MXLogDebug("[MXKRoomDataSource] Destroy %p - room id: %@", self, roomId)
                ...
                if self.room != nil {
                    ...
                    if isLive {
                        ...
                        self.room?.liveTimeline({ [self] liveTimeline in
                            ...
                            refreshEventListeners(filterMessagesWithURL ? [kMXEventTypeStringRoomMessage] : MXKAppSettings.standard().allEventTypesForMessages)
                    ...
                    MXLogDebug("[MXKRoomDataSource] Warning: The user does not know the room %@", roomId)
    ...
    func refreshEventListeners(_ liveEventTypesFilterForMessages: [AnyHashable]?) {
        ...
        redactionListener = timeline?.listen(toEventsOfTypes: [kMXEventTypeStringRoomRedaction], onEvent: { [self] redactionEvent, direction, roomState in
            ...
            if direction == MXTimelineDirectionForwards {
                ...
                MXKRoomDataSource.processingQueue().async(execute: { [self] in
    ...
    func paginate(_ numItems: Int, direction: MXTimelineDirection, onlyFromStore: Bool, success: @escaping (_ addedCellNumber: Int) -> Void, failure: @escaping (_ error: Error?) -> Void) {
        ...
        paginationListener = timeline?.listen(toEventsOfTypes: (filterMessagesWithURL ? [kMXEventTypeStringRoomMessage] : MXKAppSettings.standard().allEventTypesForMessages), onEvent: { [self] event, direction2, roomState in
    ...
    func paginate(toFill rect: CGRect, direction: MXTimelineDirection, withMinRequestMessagesCount minRequestMessagesCount: Int, success: @escaping () -> Void, failure: @escaping (_ error: Error?) -> Void) {
        MXLogDebug("[MXKRoomDataSource] paginateToFillRect: %@", NSCoder.string(for: rect))
                ...
                while i >= 0 {
                    ...
                    if bubbleHeight != 0.0 {
                        ...
                        if bubblesTotalHeight > rect.size.height {
                            ...
                            MXLogDebug("[MXKRoomDataSource] -> %tu already loaded bubbles (%tu events) are enough to fill the screen", (bubbles?.count ?? 0) - i, eventsCount)
                            break
                        }
    ...
    func send(_ image: UIImage?, success: @escaping (String?) -> Void, failure: @escaping (Error?) -> Void) {
        ...
        if room?.summary.isEncrypted ?? false {
            ...
            thumbnail = MXKTools.reduce(image, toFitInSize: CGSize(width: 800, height: 600))
            if thumbnail == image {
                thumbnail = nil
            }
        }
    ...
    /// the event id of the event generated on the homeserver
    ///   - failure: A block object called when the operation fails.
    func sendAudioFile(_ audioFileLocalURL: URL?, mimeType: , success: @escaping (String?) -> Void, failure: @escaping (Error?) -> Void) {
        var localEchoEvent: MXEvent? = nil
            ...
            if (msgType == kMXMessageTypeText) || (msgType == kMXMessageTypeEmote) {
                ...
                if contentURL != nil && contentURL?.hasPrefix(kMXMediaUploadIdPrefix) ?? false {
                    ...
                    if let image = image {
                        ...
                        failure(NSError(domain: MXKRoomDataSourceErrorDomain, code: MXKRoomDataSourceError.resendGeneric.rawValue, userInfo: nil))
                ...
                if !FileManager.default.fileExists(atPath: localFilePath) {
                    failure(NSError(domain: MXKRoomDataSourceErrorDomain, code: MXKRoomDataSourceError.resendInvalidLocalFilePath.rawValue, userInfo: nil))
                ...
                if contentURL != nil && contentURL?.hasPrefix(kMXMediaUploadIdPrefix) ?? false {
                    ...
                    failure(NSError(domain: MXKRoomDataSourceErrorDomain, code: MXKRoomDataSourceError.resendInvalidMessageType.rawValue, userInfo: nil))
                } else {
                    // Resend the Matrix event by reusing the existing echo
                    room?.sendMessage(withContent: event?.content, localEcho: &event, success: success, failure: failure)
                }
    ...
    func queueEvent(forProcessing event: MXEvent?, with roomState: MXRoomState?, direction: MXTimelineDirection) {
        ...
        if MXKAppSettings.standard().hidePreJoinedUndecryptableEvents && direction == MXTimelineDirectionBackwards {
                ...
                if (eventsToProcessSnapshot?.count ?? 0) != 0 {
                    ...
                    for queuedEvent in eventsToProcessSnapshot ?? [] {
                        ...
                        autoreleasepool {
                            ...
                            let `class` = cellDataClass(forCellIdentifier: kMXKRoomBubbleCellDataIdentifier)
                            assert(`class` is MXKRoomBubbleCellDataStoring, "MXKRoomDataSource only manages MXKCellData that conforms to MXKRoomBubbleCellDataStoring protocol")
