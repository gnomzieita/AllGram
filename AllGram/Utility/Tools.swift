...
class Tools: NSObject {
    ...
    class func isEmojiString(_ string: String?, singleEmoji: Bool) -> Bool {
        ...
        if singleEmoji && string.count > 1 {
            return false
        }
 
        for theChar in Array(string) {
            var isEmoji = false
            let words = Array(theChar.utf16)
            let hs = words[0]
            if words.count == 1 {
                // Non surrogate
                ...
                let ls = words[1]
                if 0xd800 <= hs && hs <= 0xdbff && 0xdc00 <= ls && ls <= 0xdfff {
                    // surrogate pair
...
//            }
//        }
        msisdn = "\(countryCode ?? "[cc]")\(phoneNumber ?? "[no phone number]")"
        return msisdn
    }
...
//        return argbValue
    ...
    static func availableCompressionSizes(for image: UIImage?, originalFileSize: Int) -> MXKImageCompressionSizes {
        let smallS = MXKImageCompressionSize(imageSize: CGSize(width: 100, height: 100), fileSize: 100000)
        let compressionSizes = MXKImageCompressionSizes(small: smallS, medium: smallS, large: smallS, original: smallS, actualLargeSize: 1000)
    ...
    class func paint(_ image: UIImage?, with color: UIColor?) -> UIImage? {
        ...
        UIGraphicsBeginImageContextWithOptions(image?.size ?? CGSize.zero, Bool(truncating: 0), 0)
    ...
    class func convertImage(toPatternColor resourceName: String?, backgroundColor: UIColor?, patternSize: CGSize, resourceSize: CGSize) -> UIColor? {
        ...
        if bgColor == nil {
            ...
            // set to the top quality
            guard let context = UIGraphicsGetCurrentContext() else { return backgroundColor }
            context.interpolationQuality = CGInterpolationQuality.high
...
//                    if UIApplication.openSettingsURLString != "" {
//                        alert.addAction(
//                            UIAlertAction(
//                                title: "settings",
//                                style: .default,
...
//                    viewController?.present(alert, animated: true)
    ...
    static func checkAccess(
        ...
        Tools.checkAccess(forMediaType: AVMediaType.audio.rawValue, manualChangeMessage: manualChangeMessageForAudio) { granted in

            if granted {
                // Check camera permission in case of video call
                if isVideoCall {
                    Tools.checkAccess(forMediaType: AVMediaType.video.rawValue, manualChangeMessage: manualChangeMessageForVideo) { granted in
    ...
    static func checkAccess(
        ...
        let cbStatus = CNContactStore.authorizationStatus(for: .contacts)
        ...
        } else if cbStatus == .denied && manualChangeMessage != nil {
...
//            viewController?.present(alert, animated: true)
    ...
    class func sanitiseHTML(
        ...
        imageHandler: ((_ sourceURL: String?, _ width: CGFloat, _ height: CGFloat) -> String)?
    ) -> String? {
        var html = htmlString
...
//        // Trim trailing whitespace and newlines in the string content
//        if let str = mutableAttributedString {
//            while str.string.hasSuffixCharacter(from: CharacterSet.whitespacesAndNewlines) {
...
//        mutableAttributedString?.enumerateAttributes(in: NSRange(location: 0, length: mutableAttributedString?.length ?? 0), options: [] ) { attrs, range, stop in
//
...
//        return mutableAttributedString
    ...
    class func createLinks(in attributedString: NSAttributedString?, forEnabledMatrixIds enabledMatrixIdsBitMask: Int) -> NSAttributedString? {
        ...
        return (postRenderAttributedString ?? attributedString)
    }

    class func createLinks(in attributedString: NSAttributedString?, matchingRegex regex: NSRegularExpression?, withWorking mutableAttributedString: inout NSMutableAttributedString?) {
        var linkMatches: [AnyHashable]?

        guard let attributedString = attributedString else { return }
        
        // Enumerate each string matching the regex
        regex?.enumerateMatches(in: attributedString.string, options: [], range: NSRange(location: 0, length: attributedString.length), using: { match, flags, stop in
            ...
            if !hasAlreadyLink {
                if linkMatches == nil {
                    ...
                    linkMatches = httpLinksRegex?.matches(in: attributedString.string, options: [], range: NSRange(location: 0, length: attributedString.length))
                }
            ...
            if !hasAlreadyLink {
                ...
                if let range1 = match?.range {
                    let rangeInString = Range(range1, in: attributedString.string)
                    if let link = attributedString.string[rangeInString!].addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
                    {
                        mutableAttributedString?.addAttribute(.link, value: link as NSString, range: range1)
                    }
                        ...
                        if dtTextBlock?.backgroundColor == kMXKToolsBlockquoteMarkColor {
                            ...
                            if range.location > 0 {
                                ...
                                if paragraphStyle != nil && effectiveRange.length == 1 && paragraphStyle?.firstLineHeadIndent != 25 {
                                    ...
                                    if let newParagraphStyle = paragraphStyle as? NSMutableParagraphStyle {
    ...
    class func log(forPushToken pushToken: Data?) -> String? {
        ...
        if let subdata = pushToken?[0..<len] {
            return "\(subdata)..."
        }
        return nil
    }
