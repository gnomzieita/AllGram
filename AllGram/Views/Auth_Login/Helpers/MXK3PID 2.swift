...
class MXK3PID {
    ...
    func requestValidationToken(
        ...
        if validationState != .tokenRequested && restClient != nil {
            ...
            if medium?.isEqual(kMX3PIDMediumEmail) != nil {
                ...
                print("[MXK3PID] requestValidationToken: is not supported for this 3PID:", address as Any, medium as Any)
            }
        } else {
            print("[MXK3PID] Failed to request validation token for 3PID:", address as Any, medium as Any, validationState.rawValue)
        }
    }
    ...
    func submitValidationToken(
        ...
        if validationState == .tokenReceived {
            if let submitUrl = submitUrl {
                ...
                print("[MXK3PID] Failed to submit validation token for 3PID, identity service is not set", address as Any, medium as Any)

                failure(nil)
            }
        } else {
            print("[MXK3PID] Failed to submit validation token for 3PID", address as Any, medium as Any, validationState.rawValue)
    ...
    func add3PID(
        ...
        if medium?.isEqual(kMX3PIDMediumEmail) != nil || medium?.isEqual(kMX3PIDMediumMSISDN) != nil {
            ...
            print("[MXK3PID] bindWithUserId: is not supported for this 3PID", address as Any, medium as Any)
        }
